...
const RuhSagligiNewsScreen = ({ navigation }) => {
  ...
  const fetchNews = async () => {
    setLoading(true);
    const RuhSagligiNews = await getArticlesByCategory("Ruh Sağlıgı");
    setNewsData(RuhSagligiNews);
    setLoading(false);
    console.log("First 3 articles:", RuhSagligiNews.slice(0, 3).map(a => ({
        id: a.id,
        title: a.title,
        hasImage: !!a.image,
        imageUrl: a.image
...
export default RuhSagligiNewsScreen;
