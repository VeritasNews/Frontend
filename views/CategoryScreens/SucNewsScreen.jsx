...
const SucNewsScreen = ({ navigation }) => {
  ...
  const fetchNews = async () => {
    setLoading(true);
    const SucNews = await getArticlesByCategory("Suç");
    setNewsData(SucNews);
    setLoading(false);
    console.log("First 3 articles:", SucNews.slice(0, 3).map(a => ({
        id: a.id,
        title: a.title,
        hasImage: !!a.image,
        imageUrl: a.image
