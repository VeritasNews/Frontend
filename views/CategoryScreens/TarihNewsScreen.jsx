...
const TarihNewsScreen = ({ navigation }) => {
  ...
  const fetchNews = async () => {
    setLoading(true);
    const TarihNews = await getArticlesByCategory("Tarih");
    setNewsData(TarihNews);
    setLoading(false);
    console.log("First 3 articles:", TarihNews.slice(0, 3).map(a => ({
        id: a.id,
        title: a.title,
        hasImage: !!a.image,
        imageUrl: a.image
...
export default TarihNewsScreen;
