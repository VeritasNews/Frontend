...
const SeyahatNewsScreen = ({ navigation }) => {
    const [newsData, setNewsData] = useState([]);  // ✅ Store fetched news
    const [loading, setLoading] = useState(true);  // ✅ Loading state
    const [portrait, setPortrait] = useState(isPortrait());

    useEffect(() => {
        fetchSeyahatNews();  // ✅ Fetch Seyahat category news when component mounts
    ...
    // ✅ Fetch only "Seyahat" category news
    const fetchSeyahatNews = async () => {
        setLoading(true);
        const seyahatNews = await getArticlesByCategory("Seyahat");
        setNewsData(seyahatNews);
        setLoading(false);
    };
...
export default SeyahatNewsScreen;